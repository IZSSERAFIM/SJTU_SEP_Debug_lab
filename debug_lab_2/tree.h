...
public:
  ...
  void borrow_key_from_right_leaf(node_t *node, node_t *next) {
    ...
    for (int i = 0; i < node->up->down.size(); i++) {
      if (node->up->down[i] == next) {
        node->up->key_list[i] = next->key_list.front();
        break;
      }
    }
  }
  ...
  void merge_node_with_left_leaf(node_t *node, node_t *prev) {
    node->key_list.insert(node->key_list.begin(), prev->key_list.begin(),
                          prev->key_list.end());
    node->value_list.insert(node->value_list.begin(), prev->value_list.begin(),
                            prev->value_list.end());
